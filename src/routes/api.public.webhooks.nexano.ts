  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        const providedToken = request.headers.get("x-webhook-secret") ?? request.headers.get("authorization")?.replace(/^Bearer\s+/i, "");
        ...
        const eventType = typeof payload.event === "string" ? payload.event : typeof payload.type === "string" ? payload.type : "unknown";
        const externalId = typeof payload.external_id === "string" ? payload.external_id : null;
        const customerEmail = typeof payload.customer_email === "string" ? payload.customer_email : null;
        ...
        if (insertError) {
          console.error("[Nexano Webhook] Falha ao registrar evento:", insertError);
        ...
        // --- Retorno 200 (processamento pendentemente será implementado) ---
