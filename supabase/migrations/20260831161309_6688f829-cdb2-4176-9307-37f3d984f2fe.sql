...
BEGIN
  ...
  IF _dedupe_key IS NOT NULL THEN
    ...
    IF v_id IS NULL THEN
      ...
      -- Retomada exclusiva de evento que falhou no processamento.
      UPDATE public.billing_events AS be
         SET status = 'received', error_message = NULL, processed_at = NULL
       WHERE be.id = v_id AND be.status = 'error'
      RETURNING be.id INTO v_retry;
